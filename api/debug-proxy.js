...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      console.log(
        `Debug Proxy: Logging JSON request, size ${responseSize} bytes`,
      );
      try {
        const logResult = await logger.logRequest(
          req,
          targetUrl,
          startTime,
          response.status,
        ...
        console.log(
          `Debug Proxy: Log result: ${logResult ? "Success" : "Failed"}`,
        );
      } catch (logError) {
      ...
      // Log before returning
      console.log(
        `Debug Proxy: Logging image request, size ${responseSize} bytes`,
      );
      try {
        const logResult = await logger.logRequest(
          req,
      ...
      // Log before returning
      console.log(
        `Debug Proxy: Logging text request, size ${responseSize} bytes`,
      );
      try {
        const logResult = await logger.logRequest(
          req,
